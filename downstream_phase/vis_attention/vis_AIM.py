...
def visualize_attention(input_seq, output_seq, attention, name, id, video_name):
    save_path = '/Users/yangshu/Documents/PETL4SurgVideo/save_vis/' + video_name
...
class Block(nn.Module):
    ...
    def forward(self, x, id, video_name):
        ...
        if self.num_tadapter == 2:
            if self.attn_divided:
                xt = self.T_Adapter(
                    self.temporal_attn(self.T_Adapter_in(self.temporal_norm1(xt)))
                )
            else:
                xt = self.T_Adapter(self.attn(self.T_Adapter_in(self.norm1(xt))))
        else:
            if self.attn_divided:
                xt = self.T_Adapter(self.temporal_attn(self.temporal_norm1(xt)))
            else:
                xt = self.T_Adapter(self.attn(self.norm1(xt), id, video_name))
...
if __name__ == "__main__":
    ...
    best_pretrained_path = "/Users/yangshu/Documents/PETL4SurgVideo/AIM_checkpoint-best.pth"
