...
def visualize_attention(input_seq, output_seq, attention, name, id, video_name):
    save_path = '/Users/yangshu/Documents/PETL4SurgVideo/save_vis/LA/' + video_name
    ...
    if len(input_seq) < 100:
        ...
        ax.set_yticks(range(len(output_seq)))
        ...
        for i in range(len(output_seq)):
            for j in range(len(input_seq)):
                ax.text(
                    j, i, f"{attention[i, j]:.2f}", va="center", ha="center", color="black"
                )
    ...
    plt.colorbar(cax)
    # plt.show()
    plt.savefig(save_path + '/' + str(id) + name + '.png', dpi=300)
...
class Attention(nn.Module):
    ...
    def forward(self, x, id, video_name):
        B, N, C = x.shape
        if not self.qkv_divided:
            qkv = (
                self.qkv(x)
        ...
        if attn.shape[2] == 197:
            input_sequence = [str(i) for i in range(1, 197)]
            output_sequence = [str(i) for i in range(1, 197)]
            attention_scores = attn[:,:,1:,1:].mean(0).mean(0).detach().numpy()
            visualize_attention(input_sequence, output_sequence, attention_scores, name='spatial', id=id, video_name=video_name)
        else:
            length = attn.shape[2]
            input_sequence = [str(i) for i in range(1, length+1)]
            output_sequence = [str(i) for i in range(1, length+1)]
            attention_scores = attn.mean(0).mean(0).detach().numpy()
            visualize_attention(input_sequence, output_sequence, attention_scores, name='temporal', id=id, video_name=video_name)
...
class Temporla_Adapter(nn.Module):
    def __init__(
        self, D_features, mlp_ratio=0.25, act_layer=nn.GELU, skip_connect=True
    ):
        super().__init__()
...
class Block(nn.Module):
    def __init__(
        ...
        self.scale = scale
        self.T1_Adapter = Temporla_Adapter(dim, skip_connect=False)
        self.T2_Adapter = Adapter(dim, skip_connect=False)
    ...
    def forward(self, x, id, video_name):
        ...
        xt = self.attn(self.norm1(xt), id, video_name)
        xt_agg = self.T1_Adapter(xt)
        xt_single = self.T2_Adapter(xt)
        xt = xt_agg + xt_single

        xt = rearrange(xt, "(b n) t d -> (b t) n d", n=n)
        x = x + self.drop_path(xt)
        ## spatial adaptation
        x = x + self.S_Adapter(self.attn(self.norm1(x), id, video_name))
        ## joint adaptation
        xn = self.norm2(x)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, names):
        name = names[0]
        B, C, T, H, W = x.shape
    ...
    def forward(self, x, names):
        x = self.forward_features(x, names)
        x = self.head(x)
        return x
...
if __name__ == "__main__":
    ...
    best_pretrained_path = "/Users/yangshu/Documents/PETL4SurgVideo/LA_checkpoint-best.pth"
    ...
    sampler_test = torch.utils.data.RandomSampler(dataset_test, replacement=True, num_samples=300)
    ...
    for k in data_loader_test:
        images, gt, names, _ = k
        output = model(images, names)