...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, num_frames: int) -> torch.Tensor:
        ...
        x = rearrange(x, "(b t) n d -> n (b t) d", t=num_frames)
        x = x + self.attention(self.ln_1(x))
        x = rearrange(x, "n (b t) d -> (b t) n d", t=num_frames)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat(
            [self.class_embedding.view(1, 1, -1).expand(x.shape[0], -1, -1), x], dim=1
        )  # [*, grid ** 2 + 1, width]
        ...
        x = self.ln_post(x)
        x = x[:, 0]
        x = x.contiguous().view(B, T, x.size(-1))
...
def clip_vit_base_patch16_adapter24x384(**kwargs):
    ...
    print(model.load_state_dict(clip_model.visual.state_dict(), strict=False))
    trainable_params = []
...
if __name__ == "__main__":
    ...
    model = clip_vit_base_patch16_adapter24x384(num_classes=7)
